...
+(instancetype)show:(NSString *)message{
    
    return [self showCardAlertWithTitle:nil message:message duration:3.0f hideOnSwipe:TRUE hideOnTap:TRUE alertType:ISAlertTypeWarning alertPosition:ISAlertPositionTop didHide:nil];
}

+(instancetype)show:(NSString *)message duration:(NSTimeInterval)duration didHide:(completion)didHide {
    
    return [self showCardAlertWithTitle:nil message:message duration:duration hideOnSwipe:TRUE hideOnTap:TRUE alertType:ISAlertTypeWarning alertPosition:ISAlertPositionTop didHide:didHide];
}

+(instancetype)show:(NSString *)message position:(ISAlertPosition)position {
    return [self showCardAlertWithTitle:nil message:message duration:3.0f hideOnSwipe:TRUE hideOnTap:TRUE alertType:ISAlertTypeWarning alertPosition:position didHide:nil];
}

+(instancetype)show:(NSString *)message type:(ISAlertType)type{
    
    return [self showCardAlertWithTitle:nil message:message duration:3.0f hideOnSwipe:TRUE hideOnTap:TRUE alertType:type alertPosition:ISAlertPositionBottom didHide:nil];
}

+(instancetype)show:(NSString *)message type:(ISAlertType)type iconImage:(UIImage*)iconImage{
    
    ISMessages* alert = [self cardAlertWithTitle:nil message:message iconImage:iconImage duration:3.0f hideOnSwipe:TRUE hideOnTap:TRUE alertType:type alertPosition:ISAlertPositionTop];
...
- (void)constructAlertCardView {
    ...
    if(_messageString.length > 0) {
        ...
        if(_titleString.length == 0) {
            [messageLabel setCenter:CGPointMake(messageLabel.center.x, self.view.frame.size.height/2)];
        }
    }
...
-(UIColor *) Successcolor {
    return [UIColor colorWithRed:71.f/255.f green:158.f/255.f blue:39.f/255.f alpha:1.f];
}
-(UIColor *) Errorcolor {
    return [UIColor colorWithRed:215.f/255.f green:35.f/255.f blue:35.f/255.f alpha:1.f];
}
-(UIColor *) Warningcolor {
    return [UIColor colorWithRed:0.f/255.f green:0.f/255.f blue:0.f/255.f alpha:0.8f];
    //return [UIColor colorWithRed:255.f/255.f green:144.f/255.f blue:0.f/255.f alpha:1.f];
}
-(UIColor *) Infocolor {
return [UIColor colorWithRed:69.f/255.f green:97.f/255.f blue:115.f/255.f alpha:1.f];
}
-(UIColor *) Customcolor {
...
- (void)configureViewForAlertType:(ISAlertType)alertType iconImage:(UIImage*)iconImage {
    ...
    switch (alertType) {
        case ISAlertTypeSuccess: {
            self.alertViewBackgroundColor = [self Successcolor];
            if (!_iconImage) {
                self.iconImage = [self imageNamed:@"success_icon"];
            }
            break;
        }
        case ISAlertTypeError: {
            self.alertViewBackgroundColor = [self Errorcolor];
            if (!_iconImage) {
                self.iconImage = [self imageNamed:@"error_icon"];
            }
            break;
        }
        case ISAlertTypeWarning: {
            self.alertViewBackgroundColor = [self Warningcolor];
            if (!_iconImage) {
                self.iconImage = [self imageNamed:@"warning_icon"];
            }
            break;
        }
        case ISAlertTypeInfo: {
            self.alertViewBackgroundColor = [self Infocolor];
            if (!_iconImage) {
                self.iconImage = [self imageNamed:@"info_icon"];
            }
            break;
        }
        case ISAlertTypeCustom: {
            self.iconImage = [self imageNamed:@"appIcon"];
            self.alertViewBackgroundColor = [self Customcolor];
